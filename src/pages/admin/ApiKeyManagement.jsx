...
export default function ApiKeyManagement() {
  ...
  async function testKey(index) {
    const keyStr = keys[index].key.trim();
    if (!keyStr) return error('Please enter a key first.');
    
    setTesting(prev => ({ ...prev, [index]: true }));
    try {
      const gemini = (await import('../../services/gemini')).default;
      const result = await gemini.isKeyValid(keyStr);
      if (result.valid) {
        success('Key is valid and working!');
      } else {
        error(`Key failed: ${result.error}`);
      }
    } catch (err) {
  ...
  async function handleSave() {
    ...
    if (validKeys.length === 0) return error('Please add at least one key.');

    setSaving(true);
    info('Saving and verifying keys...');
    
    try {
      // Final check of the first active key to make sure something works
      const firstActive = validKeys.find(k => k.enabled);
      if (firstActive) {
        const gemini = (await import('../../services/gemini')).default;
        const result = await gemini.isKeyValid(firstActive.key);
        if (!result.valid) {
          error(`The primary key seems restricted/invalid: ${result.error}. Saving anyway...`);
        }
      }
          ...
          {keys.map((keyObj, i) => (
            <div key={i} className="glass-card" style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.875rem 1rem' }}>
              ...
              <button 
                ...
                onClick={() => testKey(i)} 
                disabled={testing[i]}
                style={{ color: 'var(--accent-indigo)', minWidth: 60 }}
              >
                {testing[i] ? <RefreshCw size={14} className="spin" /> : 'Test'}
              </button>
